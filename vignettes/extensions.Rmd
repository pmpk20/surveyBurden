...
pass it the scored catalogue and a paths table with one row per route:
`path_id`, `terminates_early`, `block_ids` (list), `q_always` (list of
question ids always shown on that route), `q_maybe` (list of question ids
...
routes and deciding, for each, which questions are always or maybe shown.
