...
**What this answers.** What each section of the printed `burden_report()` output shows, and how to read it.

**What you need.** A report from `burden_report()` on your survey's `.qsf`.

**What it cannot establish.** The report describes the modelled burden of the instrument. It does not predict completion time, dropout or data quality, and its median is not the burden half of respondents face.
...
Structural counts read straight from the survey (editing URL or QSF): live
...
The printed "Display-logic trigger questions" line gives the range, across
complete paths, of `n_gates`: the number of distinct questions on a path
whose answers decide whether some other question is shown. It is a count
of trigger questions, not of the display-logic combinations enumerated;
one trigger question with five answer options can produce several
combinations.
...
display-logic question, how many conditional questions were enumerated in
full (every state the model represents) versus approximated, whether every
loop has a known cap, and the `auto` / `inferred` / `manual` / `unknown`
split of the item scores. These are modelling diagnostics, not a
certificate that the result matches the live survey: branch conditions,
embedded-data conditions and unusual Qualtrics constructs are outside what
they check. A result can be fully reproducible without every underlying
interpretation being certain; this block makes the uncertain parts
countable. Pass `certainty = FALSE` to skip it.
...
- `vignette("survey-revision")` -- act on the heaviest blocks and questions.
- `vignette("paths-and-display-logic")` -- how the paths and the burden spread are built.
- `vignette("calibration")` -- the points-to-minutes assumption and the limitations.
