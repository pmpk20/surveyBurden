# Extending surveyBurden to other survey platforms
...
surveyBurden currently reads Qualtrics `.qsf` files. Its internal
architecture, however, separates the platform-specific parser from the
platform-agnostic scoring and analysis layers. This vignette documents
that boundary so that someone who wanted to add support for another
platform (LimeSurvey, SurveyMonkey, REDCap, etc.) could write a new
...
**Layer 1** reads a Qualtrics survey definition and produces a
...
platform-specific. A new platform would need to provide an equivalent
flow structure or an adapter (see *What a new parser must provide*
below).

**Layer 3** applies the GfS scoring rules to the catalogue, computes
...
The package does not claim to be platform-agnostic. It is a
Qualtrics-first tool whose architecture *supports extension* to other
platforms. The three-layer design means that adding a new platform does
not require touching the scoring rules, the path enumeration algebra, or
the reporting logic. What it does require is:
