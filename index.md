...
surveyBurden scores every question with the published GfS / Axhausen
burden scheme (Heimgartner & Axhausen 2024, Table 1), then follows the
survey’s flow and display logic to work out which questions can appear
together and how burden varies across the feasible respondent paths. The
score for one question is its **question burden**; the total along one
route is a **path burden**. The default result is a **structural burden
profile**: each feasible path and display-logic sub-state is counted
once, so the reported “median” is the middle value across those
combinations, not the burden half of respondents exceed. Given real
respondent route data, the report instead reports a
