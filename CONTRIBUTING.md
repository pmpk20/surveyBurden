...
Open an issue at <https://github.com/pmpk20/surveyBurden/issues>. For a
bug, please include:

- the `surveyBurden` version (`packageVersion("surveyBurden")`) and your
  R version (`R.version.string`);
- a minimal `.qsf` (or a redacted fragment) that reproduces the problem,
  or the exact
...
`.qsf` files can contain question wording you may not want public — a
fragment that still triggers the bug, or an emailed file, is fine.
...
4.  Follow the existing style (the tidyverse style guide; `styler` and
    `lintr` are configured).
