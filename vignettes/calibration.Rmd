...
12). It is a practical proxy, not a calibration, and it is the reason the
descriptive-text score for a long instruction block should be read as
approximate.
...
completion-time data with `validate_times()` and use the rate it
returns. This needs a completion-time file the package does not ship, so
it is not run here:
