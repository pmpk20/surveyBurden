...
compute the **realised burden**: the GfS+ points each respondent actually
incurred, given the questions they answered, the display-logic branches
they took and the loop iterations they completed.
...
| `realised_burden()`   | QSF + response data | Per-respondent burden    | *ex post* |

Use `respondent_burden()` to evaluate a draft survey before fielding.
Use `realised_burden()` after fielding to see how burden was actually
